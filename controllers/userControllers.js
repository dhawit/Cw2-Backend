...
const sendOTP = async (req, res) => {
  try {
    ...
    await transporter.sendMail({
      from: '"Dhawit Tamang" <tdhawit@gmail.com>',
      to: email,
...
const sendVerificationEmail = async (user) => {
  ...
  await sendEmail(
    ...
    <a href="${verificationUrl}" style="padding: 10px 20px; background-color: #ff8b8b; color: #fff; text-decoration: none; border-radius: 5px;">Verify Email</a>
  `
  );
};
...
const verifyEmail = async (req, res) => {
  ...
  console.log('Verification request received with id:', id, 'and token:', token);
...
const editUserProfile = async (req, res) => {
  try {
    ...
    const updatedUserProfile = await User.findByIdAndUpdate(userId, req.body, {
      new: true,
      runValidators: true,
    });
    res.status(200).json({
